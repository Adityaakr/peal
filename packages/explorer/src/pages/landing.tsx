  ...
  return (
    <div className={`peal-committee${opens ? ' is-open' : ''}`}>
      ...
      <p className="peal-cmt-cap">
        ...
        {opens
          ? 'shares arrived, so it opened. Any three will do, and it does not matter which three.'
          : 'is not enough. Turn on one more and it opens; it does not matter which.'}
        {' '}This shows the threshold rule, not who runs the five.
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <p>
        ...
        submission opens at once. None of them is trusted: two together learn nothing, and every
        share is checked by a public pairing equation. The reveal is not a participant&rsquo;s
        move, so there is nothing to withhold.
      </p>
    </Section>
  );
}
...
function Committee() {
  return (
    ...
    >
      <p>
        No single operator holds a key that opens anything, and no operator is trusted. The key is
        split across five independent operators and takes three of them to use. Two can be
        offline, compromised or simply refusing, and the round still opens on time.
      </p>
      <CommitteeScene />
      <p>
        The threshold is arithmetic, so it is not a diagram. Turn operators off and watch: two
        never opens it, any three do.
      </p>
      <p className="peal-fine">
        Independent is the design and the mainnet commitment. Who runs the five today is a separate
...
const FAQS: Array<[string, React.ReactNode]> = [
  ...
  [
    ...
    <>
      Nobody, and nobody has to be trusted for that: the decryption key is split across five independent operators, no three of them combine their shares until the condition fires, and two of them together learn nothing. On the devnet running today those five are run by us, from one deployment, with a dealer-generated key, so the threshold is real and the independence is not yet. Mainnet is five named, independently operated members with keys generated by DKG, listed here by name before the network carries value.
    </>,
  ],
  [
  ...
  [
    ...
    <>
      ...
      testnet, and the parameters and endpoints are published. It is a devnet: the five operators
      are all run by us, and none of it is carrying real money yet.
    </>,
  ],
  [
    ...
    <section className="peal-sec peal-close">
      <div className="peal-sec-inner">
        ...
        <p className="peal-close-note peal-close-fine">
          ...
          documented; the committee parameters are published and its five operators are ours
          today. None of it is carrying real money yet.
        </p>
      </div>
    </section>
  );
