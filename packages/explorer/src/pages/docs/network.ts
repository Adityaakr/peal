 ...
 *
 * Region is the one thing that sounds like tracking and is not. It is read from
 * a header the edge already set, at a resolution of a handful of metros. No
 * address is parsed, stored or sent anywhere. See region_of in activity.rs.
 */
...
interface Activity {
  ...
  timings?: number[];
  tags: TagRow[];
  regions: { code: string; calls: number }[];
  ...
  mount: (root) => {
    ...
    const unavailable = (): void => {
      ...
      for (const id of ['#act-lines', '#act-installs', '#act-latency', '#dev-board',
                        '#act-donut', '#act-regions', '#act-endpoints']) {
        const node = el(id);
