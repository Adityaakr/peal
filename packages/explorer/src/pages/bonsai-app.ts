...
function topbar(): string {
  ...
  const who = evm.address
    ? `<a class="pla-chip pla-chip-wallet" href="${hashFor('settings')}" title="${evm.source === 'privy' ? 'Privy wallet' : 'browser wallet'}"><span class="pla-avatar" aria-hidden="true">${esc(evm.address.slice(2, 4))}</span><span class="pla-chip-text"><b>${esc(page.displayName ?? shortHex(evm.address, 6, 4))}</b><i>${evm.source === 'privy' ? 'Privy wallet' : 'browser wallet'} · ${esc(shortHex(evm.address, 6, 4))}</i></span></a>`
    : '';
  return `
