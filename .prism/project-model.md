...
## Peal Links (Bonsai private payments), added 2026-09-16
- Branch `feat/peal-links`. Spec `docs/peal-links/SPEC.md`; living log `docs/peal-links/BUILD_STATUS.md` (gates A to F passed); decisions `docs/peal-links/decisions/0001..0010`.
- Foundation: `zkpari` git dep pinned at PR #2 head `a8266aac` (Poseidon, depth 32/32, R_op only). Core crate `crates/peal-bonsai`; node `crates/peal-links-node` (:8790, `/links/v1`); wasm `crates/peal-links-wasm`; SDK `packages/links`; gateway `contracts/src/links/PealLinksGateway.sol`; pages `packages/explorer/src/pages/{bonsai-landing,bonsai-app,pay}.ts`.
- Invariants (tested): ledger STF verifies every proof under the pinned vk (no other acceptance path); old commitment must equal current (`StaleCommitment`); recent-root window is a ring checked in `apply_verified` for single, batched and replayed application (decision 0009); deposits credited only from finalized gateway events matching a registered R_dep intent, deduplicated by `chain:tx:logIndex`; withdrawals are sends to `withdraw_receiver()` whose opening is consumed once and certified by the committee (EIP-712 digest cross-checked with Foundry `deployCodeTo`).
- Danger zones: `.dev-params/` (regenerating changes the circuit id and invalidates every wallet; reset data with it); arkworks emits a tracing span per constraint (node disables those targets and runs keygen and the ledger actor under `NoSubscriber`, else keygen hangs); `cargo fmt` reflows code so python string replacements must be re-checked; anvil is in-memory, `stack.sh` wipes ledger data whenever it places fresh contracts.
- Trust model of the local demo: real proofs, local setup, committee-attested bridge. Ledger: single node by default, or three local Commonware simplex validators with `PEAL_LINKS_VALIDATORS=3` (decision 0010, crate `crates/peal-links-consensus`, pinned `commonware-* =2026.9.0`). Signers: single-process fixture (single node) or one key per validator process with peers co-signing after checking their own replicated ledger. Everything labelled in `GET /links/v1/status`; `GET /links/v1/consensus` and `stack.sh consensus` show the validators agree.
- Consensus invariants (tested in `crates/peal-links-consensus/tests/consensus.rs`): votes are stateless (structure, round, parent, signatures, proofs against the claimed commitment, own-RPC deposit confirmation for mints); application at finalization goes through the same `Ledger` code and re-verifies proofs; a block digest is the SHA-256 of its wire bytes; genesis binds circuit id + namespace set; missing blocks are fetched by digest. Runtime contexts are not `Clone` in 2026.9.0: spawn from `context.child("label")`; `tokio::time` does not run on the deterministic runtime, so deadlines live in the actor's clock.
- Measured: R_op prove 521 ms (10 threads) / 1.8 s (1 thread) native, 6.5 s wasm; verify 0.94 ms; batch of 32 at 0.79 ms per op (sqlite-bound); validator mode adds one finalization round per write (idle chain about 2 blocks/s). See `docs/peal-links/BENCHMARKS.md`.
- Lesson (2026-09-16, adversarial review of the consensus crate): the happy-path suites said nothing about a malicious peer; one Opus skeptic found unbounded caches, proof verification on the actor task and a settlement double-certificate path, all fixed the same day (BUILD_STATUS.md "Adversarial review of the consensus path"). Budget a skeptic pass for any network-facing code before calling it done.

## Decision log: 2026-09-17 developer surface for Peal Private Links (prism, BUILD)
- Three developer pages (`developers/links`, `links-sdk`, `links-api`) + `skills/peal/reference/links.md`; no new API, the node's `/links/v1` was documented as is. Decision record: `docs/peal-links/decisions/0014-developer-surface.md` (telemetry inside).
- Invariants surfaced (cited there): sessions are EOA-only so the directory's ERC-1271 path is unreachable over HTTP; the request reservation is advisory; the SDK collapses 502/503/504 to `unreachable`; `DepositIntent.amount` is a JSON number on the wire; the hosted node's sign-in domain is `peal.network`, other origins need their own node.
- Lesson (measured): four explorers with disjoint file sets gave divergence 0.93; the grounding pass still found 12 factual slips in the draft (mostly omitted prerequisites such as the session). Draft from briefs, then always re-open the source.

## BTE v1: transparent setup from a DKG (2026-09-25, prism-ship)
- Two schemes, dispatched on wire magic: `BTE0` (v0, simple-bte, dealer,
  fixed B) and `BTE1` (v1, `bte_crypto::tbte`, the paper "DKG Is All You
  Need"). `crates/bte-coordinator/src/scheme.rs` is the only place that
  matches on both; everything else calls through it. Committees carry a
  `scheme` column; v1 conditions hold one batch of at most 4095 real
  ciphertexts plus one coordinator decoy (`Committee::decoys_for`).
- v1 ciphertexts commit to `tbte::condition_context(condition_id)`; the
  coordinator refuses any other context and any second ciphertext with the
  same KEM point under a condition (unique index `idx_cts_kem_point`).
- DKG: Commonware feldman_desmedt via `tbte::dkg`; N3f1 makes five
  operators 4-of-5 (t = n - floor((n-1)/3)); the coordinator is an
  untrusted relay (`/v0/dkg/*`, `dkg.rs`); nodes drive rounds with
  `bte_node::dkg_client::RoundDriver` (dealer seed = HKDF(identity, round
  digest), state rebuilt from the relay after a restart); shares live in
  `committee-<id>.share` files beside the identity, re-checked against the
  operator key on every load.
- Measured lesson: arkworks group FFTs make the paper's O(B log^2 B) cross
  terms 3.5x-7x slower than B Pippenger MSMs up to B = 1024, so
  `CrossTermStrategy::Auto` is naive below 32768; the fast path is kept,
  tested equal, and selectable. Do not "optimize" by switching the default
  without re-running `examples/tbte_crossover.rs`.
- Skeptic-found bugs in the first draft, all fixed: O(B^2) memory in the
  naive path, no batch cap, proof bound only to pk (now the params digest
  and a context), Σk = 0 stall, panic paths in helpers, no CryptoRng bound.
