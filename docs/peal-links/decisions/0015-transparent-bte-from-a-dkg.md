...
## Attack review of the integration (2026-09-25, full fleet)

A second reviewer (Opus) reproduced seven attacks against the relay, the intake and the node driver as throwaway tests, all fixed with regression tests before the ship report: dealer-seed reuse across relay round ids (the seed was derived from identity and digest; now fresh entropy persisted per digest with the round id, and the digest covers the relay id and the box keys), unauthenticated box keys (operators now sign them), anonymous committee registration becoming the default (admin token; v1 only via DKG), the Railway image's `BTE_DEV=1` waiving the admin check (waiver now requires a loopback listen address), garbage shares locking honest operators out (verified-only share slots, rejected shares in an audit table), one garbage log failing a round (checked at the door and at settlement), unbounded per-request pairing cost (index range check, admitted verification, cached headers, proofs outside the lock), the freeze race (intake closed before the batch is read), and the silently swallowed duplicate-KEM-point conflict. Accepted and recorded: public-condition squatting up to the cap, relay clock as the deadline baseline, Joint-Feldman key bias. Details in SECURITY.md.
