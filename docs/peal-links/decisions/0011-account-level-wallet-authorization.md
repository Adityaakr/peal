...
- Local private state is partitioned per wallet address (`walletScopedStore`, 2026-09-16): one browser can hold the accounts of several wallets, each unlocked only when its wallet is connected, since the address is the identity the person switches between in their wallet. Accounts saved before the partition are adopted by their owner on first use and never by another wallet.
