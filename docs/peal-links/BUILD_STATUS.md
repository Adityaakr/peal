...
Run it: `scripts/peal-links/demo.sh` (everything, about 2.5 min after the first build); or `scripts/peal-links/stack.sh up` then open http://localhost:5173/#/bonsai; validator mode: `PEAL_LINKS_VALIDATORS=3 scripts/peal-links/stack.sh reset` and `scripts/peal-links/stack.sh consensus`. Tests: `cargo test --workspace --release`, `forge test`, `pnpm -C packages/links test`, `pnpm -C packages/explorer test:e2e`. Smoke: `cargo test -p peal-bonsai --release`.
