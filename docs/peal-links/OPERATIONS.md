...
- **Service settings.** Railway no longer lets a new service name a config file, so set these in the dashboard: Settings -> Build -> Dockerfile Path `docker/Dockerfile.links`; Settings -> Deploy -> Healthcheck Path `/healthz`, timeout 600 s, restart on failure. `railway.links.json` records the same values. The repository must not carry a root `railway.json`: Railway applies one to every service without a config path of its own, and the node would be built from the devnet Dockerfile instead (this happened once; the devnet config now lives at `railway/devnet.json`). Give the service the branch the explorer builds from, and add `PORT=8790`.
